...
export class Shield {
    ...
    private physics = {
        mass: 100, friction: 1, restitution: 0.5
    }
    private sizes = {
    ...
    constructor(name: string, position: Vector3, scene: Scene) {
        ...
        this.shield = MeshBuilder.CreateBox(name, { ...this.sizes, wrap: true, updatable: true }, scene);
    ...
    appendPhysics() {
        this.aggregate = new PhysicsAggregate(this.shield, PhysicsShapeType.BOX, this.physics, this.scene);
        this.aggregate.body.setMotionType(PhysicsMotionType.ANIMATED);
    }
    private initControler() {
        this.controler = MeshBuilder.CreatePlane("shield-control-plane", { width: 5.5, height: 2.5, updatable: true }, this.scene);
    ...
    private initMaterials() {
        const shield = new StandardMaterial(`shield-mt`, this.scene);
        shield.emissiveColor = new Color3(0.7, 0.5, 0.1);
        shield.alpha = 0.8;
        this.shield.material = shield;

        const control = new StandardMaterial(`shield-control-mt`, this.scene);
    ...
    private initEffects() {
        const glow = new GlowLayer("Shield-Glow", this.scene);
        glow.addIncludedOnlyMesh(this.shield);
        glow.intensity = 2.5;
    }
}